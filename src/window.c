...
struct Window createWindow(){
    ...
    if(s_device == NULL){
        printf("Error creating gpu device: %s ",SDL_GetError());
    }
...
void newFrame(struct Window *window){
    ...
    if(window->commandBuffer == NULL){
        printf("Error aquire command buffer: %s",SDL_GetError());
    }
    ...
    if(window->renderPass == NULL){
        printf("Error begain render pass: %s",SDL_GetError());
    }
...
void endFrame(struct Window *window){
    ...
    if(!SDL_SubmitGPUCommandBuffer(window->commandBuffer)){
        printf("Error submit command buffer: %s",SDL_GetError());
    }
}